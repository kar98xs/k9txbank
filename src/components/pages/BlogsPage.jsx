...
export default function BlogsPage() {
 ...
 const onSubmit = async (e) => {
  ...
  const submitReply = async (blogId, parentId) => {
    ...
    if (!content) return;
    try {
      const created = await authService.addComment({
        blogId,
        content,
        parent: parentId,
      });
      ...
      {comments.map((c, idx) => (
        <Box key={c.id} sx={{ mb: 1.5 }}>
          <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
            ...
            />
            <Typography
              sx={{ color: "#eaf2ff", fontWeight: 600, fontSize: 14 }}
            >
              {c.author_username}
            </Typography>
          </Box>
          <Typography sx={{ color: "#bcd3ff", mb: 1, mt: 0.5 }}>
            {c.content}
          </Typography>
          {depth < 1 && (
            <Box sx={{ display: "flex", gap: 1, mb: 1 }}>
              ...
              <Button
                variant="outlined"
                onClick={() => submitReply(blogId, c.id)}
              >
                Reply
              </Button>
            </Box>
          ...
          {Array.isArray(c.replies) &&
            c.replies.length > 0 &&
            renderComments(blogId, c.replies, depth + 1)}
  ...
  return (
    ...
    >
      <Typography
        variant="h4"
        sx={{ color: "#eaf2ff", fontWeight: 800, mb: 1 }}
      >
        Blogs
      </Typography>
      <Typography sx={{ color: "#9fb4ff", mb: 3 }}>
        Share updates and read from others
      </Typography>
      <Box
        component="form"
        onSubmit={onSubmit}
      ...
      <Box
        ...
        sx={{ maxHeight: "60vh", overflow: "auto", pr: 1 }}
      >
        {blogs.map((b, idx) => (
          <Box key={b.id}>
            <Box sx={{ mb: 2 }}>
              <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                ...
                <Typography
                  variant="h6"
                  sx={{ color: "#eaf2ff", fontWeight: 700 }}
                >
                  {b.title}
                </Typography>
              </Box>
              <Typography sx={{ color: "#bcd3ff", mb: 0.5, mt: 0.5 }}>
                {b.content}
              </Typography>
              {b.image_url && (
                <Box sx={{ mt: 1 }}>
                  <Box
                    ...
                    onError={(e) => {
                      console.error("Image load failed:", {
                        originalUrl: b.image_url,
                        transformedUrl: e.target.src,
                      });
              ...
              <Typography variant="caption" sx={{ color: "#9fb4ff" }}>
              ...
              <Box sx={{ display: "flex", gap: 1, mt: 1 }}>
                <TextField
                  ...
                  onChange={(e) =>
                    setCommentInputByBlog((prev) => ({
                      ...prev,
                      [b.id]: e.target.value,
                    }))
                  }
                />
                <Button variant="outlined" onClick={() => submitComment(b.id)}>
                  Comment
                </Button>
              </Box>
              {user?.is_staff && (
